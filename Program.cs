    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                List<Scored> top = Search(question, knowledgeIndexes, topK: 3, accessToken, embeddingsUrl);
        ...
        // ─────────────────────────────────────────────────────────────────────────
        private static string AskWithContext(string question, List<Scored> context, string accessToken, string chatUrl)
        {
            ...
            return AskRaw(messages, accessToken, chatUrl);
        }
        ...
        private static List<Scored> Search(string query, List<Indexed> index, int topK, string accessToken, string embeddingsUrl)
        {
            float[] queryVector = Embed(new List<string> { query }, accessToken, embeddingsUrl)[0];
